...
function getTime(date) {
  ...
  const actualHours = hours > 9 ? `${hours}` : `0${hours}`;
  const actualMinutes = minutes > 9 ? `${minutes}` : `0${minutes}`;
  const actualSeconds = seconds > 9 ? `${seconds}` : `0${seconds}`;
...
function getNextFriday(date) {
  ...
  const multys = [5, 4, 3, 2, 1, 7, 6];
  const multDay = multys[day];
  const milliseconds =
    currentDate - new Date('01 Jan 1970 00:00:00 UTC') + 86400000 * multDay;
...
function isDateInPeriod(date, period) {
  const beginDay = new Date('01 Jan 1970 00:00:00 UTC');
  const currentDateMs = new Date(date) - beginDay;
  const startDateMs = new Date(period.start) - beginDay;
  const endDateMs = new Date(period.end) - beginDay;
  if (currentDateMs >= startDateMs && currentDateMs <= endDateMs) return true;
  return false;
}
...
function formatDate(dateStr) {
  ...
  if (hour >= 0 && hour < 12) {
    return `${month}/${date}/${fullYear}, ${hour}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} AM`;
  }
  return `${month}/${date}/${fullYear}, ${hour === 12 ? hour : hour - 12}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} PM`;
}
...
function getWeekNumberByDate(date) {
  ...
  const currantDay = currentDate.getUTCDate();
...
function getQuarter(date) {
  const currnetMonth = new Date(date).getUTCMonth();
  if (currnetMonth <= 3) {
    return 1;
  }
  if (currnetMonth <= 6) {
